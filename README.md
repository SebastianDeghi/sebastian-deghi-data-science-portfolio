...
| 🤖 **Comparativa de LLMs para Sentiment Analysis** | Evaluación de cuatro enfoques arquitectónicos para clasificación de sentimientos: BiLSTM con atención, TinyLlama 1.1B (zero-shot), GPT-Neo 1.3B (zero-shot) y BERT (DistilBERT fine-tuneado). Análisis de equilibrio entre precisión, interpretabilidad y costo computacional. | `BiLSTM` `Attention` `TinyLlama` `GPT-Neo` `BERT` `Transformers` `PyTorch` `Hugging Face` | [Repositorio](https://github.com/SebastianDeghi/imdb-llm-sentiment-analysis) |
